...
service_endpoint = "https://aisearch-auseast-1.search.windows.net"
index_name = "gcash-text-real-wc-2000-1000-all-mpnet-base-v2-400-400"
#index_name = "gcash-text-real-wc-ada-2docs-2000-1000-text-embed-ada-002-1-400-400"
...
def simple_text_query(search_text="spa"):
    ...
    search_client = SearchClient(service_endpoint, index_name, AzureKeyCredential(AZURE_SEARCH_KEY))
    results = search_client.search(search_text, top=2)

    print("Words containing 'keyword' in the name (or other fields):")
    #keys needed dict_keys(['description', 'contentDescription', 'summary', 'contentTitle', 'id', 'text', 'content', 'filename', 'title'])
    response = [{k:v for k,v in result.items() if k in ['description', 'contentDescription', 'summary', 'id', 'text', 'content', 'filename', 'title']} for result in results]
    # for result in results:
    #     {k:v for k,v in result.items() if k in ['description', 'contentDescription', 'summary', 'id', 'text', 'content', 'filename', 'title']}
    #     print(result)
    return response
    # [END simple_query]
...
async def chat(auth_claims: Dict[str, Any]):
    ...
    try:
        ...
        # search_query = request_json.get("messages", [{}])[0].get("content")
        # result = simple_text_query(search_query)
        # print(result)
...
async def setup_clients():
    ...
    os.environ["AZURE_OPENAI_KEY"] = "2925d99e7b2a4d3997e9a57e841f5a37"
    os.environ["AZURE_OPENAI_EMBEDDING_MODEL"] = "text-embedding-ada-002"
    os.environ["AZURE_SEARCH_USE_SEMANTIC_SEARCH"] =  "False"
    os.environ["AZURE_SEARCH_SEMANTIC_SEARCH_CONFIG"] =  "default"
    os.environ["AZURE_SEARCH_TOP_K"] =  "5"
    os.environ["AZURE_SEARCH_ENABLE_IN_DOMAIN"] =  "true"
    os.environ["AZURE_OPENAI_TEMPERATURE"] =  "0"
    os.environ["AZURE_OPENAI_TOP_P"] =  "1.0"
    os.environ["AZURE_OPENAI_MAX_TOKENS"] =  "1000"
    os.environ["AZURE_OPENAI_SYSTEM_MESSAGE"] =  "You are an AI assistant that helps people find information."
    os.environ["AZURE_OPENAI_API_VERSION"] =  "2023-03-15-preview"
    os.environ["AZURE_OPENAI_STREAM"] =  "true"
    os.environ["AZURE_OPENAI_MODEL_NAME"] =  "gpt-4" # Name of the model, e.g. 'gpt-35-turbo' or 'gpt-4'
    ...
    OPENAI_CHATGPT_MODEL = os.getenv("AZURE_OPENAI_CHATGPT_MODEL", 'gpt-4')
    OPENAI_EMB_MODEL = os.getenv("AZURE_OPENAI_EMB_MODEL_NAME", "text-embedding-ada-002")
    # Used with Azure OpenAI deployments
    AZURE_OPENAI_SERVICE = os.getenv("AZURE_OPENAI_SERVICE", "gcashwealthcoachpocopenai")
    ...
    OPENAI_API_KEY = os.getenv("OPENAI_API_KEY", "2925d99e7b2a4d3997e9a57e841f5a37")
    OPENAI_ORGANIZATION = os.getenv("OPENAI_ORGANIZATION")

    AZURE_TENANT_ID = os.getenv("AZURE_TENANT_ID", "fa5e196b-9582-419a-9f44-bbc8f6265df1")#"e2c7deed-258c-48bb-b5b1-9c2cf71d2b81")
    ...
    search_client = SearchClient(
        endpoint=f"https://aisearch-auseast-1.search.windows.net/",
    ...
    if OPENAI_HOST == "azure":
        ...
        openai_client = AsyncOpenAI(base_url=os.getenv("OPENAI_BASE_URL"), api_key="no-key-required")
    else:
        openai_client = AsyncOpenAI(
            api_key=OPENAI_API_KEY,
